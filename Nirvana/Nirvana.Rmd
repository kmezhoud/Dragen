...
* After the `positions` lines, there are the `gens`lines, which are optional if there is no overlapping gene of the input `VCF` variants (3 lines for 2 genes in th example)
...
genes[c(1:77)] |>
  ...
  DT::datatable(options = list(scrollX = TRUE, autoWidth = TRUE,
                               columnDefs = list(list(className = 'dt-center', targets = c(0,1, 2, 3, 4, 5,6,7, 12)), list(width = '650px', targets = c(11, 14)))
  ),
  rownames = FALSE
  ) #%>% 
  #DT::formatStyle(columns = c(9,10), width='100px')
